...
void _offsets_init(void) {
    ...
    if ([device  isEqual: @"iPhone 6S"]) {
        if (SYSTEM_VERSION_EQUAL_TO(@"15.1")) {
            //printf("[i] jailbreakd %s offsets selected for iOS 15.1\n", device.UTF8String);
...
//            off_kalloc_data_external = 0xFFFFFFF0071C5CC8;
        ...
        if (SYSTEM_VERSION_EQUAL_TO(@"15.8")) {
            ...
            off_kalloc_data_external = 0xFFFFFFF0071D480C;//done
            off_kfree_data_external =  0xFFFFFFF0071D4E78;//done
            off_add_x0_x0_0x40_ret = 0xFFFFFFF00596A60C;//done AppleARMPE
            off_empty_kdata_page = 0xFFFFFFF007820000 + 0x100;//done
            off_trustcache = 0xFFFFFFF0078B6570;//done
            off_gphysbase = 0xFFFFFFF00714E7C0;//done
            off_gphyssize = 0xFFFFFFF00714CA78;//???;//not done yet //??
            off_pmap_enter_options_addr = 0xFFFFFFF0072C9C54;// done
...
//            off_kalloc_data_external = 0xFFFFFFF0071C5CC8;
        ...
        if (SYSTEM_VERSION_EQUAL_TO(@"15.8")) {
            //printf("[i] jailbreakd %s offsets selected for iOS 15.8\n", device.UTF8String);
            off_kalloc_data_external = 0xFFFFFFF0071D4848;//done
            off_kfree_data_external =  0xFFFFFFF0071D4EB4;//done
            off_add_x0_x0_0x40_ret = 0xFFFFFFF00596A60C;//done
            off_empty_kdata_page = 0xFFFFFFF007824000 + 0x100;//done
        ...
        if (SYSTEM_VERSION_EQUAL_TO(@"15.2")) {
           ...
           off_zm_fix_addr_kalloc = 0xFFFFFFF00713E598;
            ...
            off_empty_kdata_page = 0xFFFFFFF00959C000 + 0x100;//// + 0x100;
            off_trustcache = 0xFFFFFFF00979BF80;//;
            off_gphysbase = 0xFFFFFFF0078803D8;//
            off_gphyssize = 0xFFFFFFF0078803F0;//
            off_pmap_enter_options_addr = 0xFFFFFFF007CA2718;//;
            ...
            off_add_x0_x0_0x40_ret = 0xFFFFFFF0083B7848;
            off_empty_kdata_page = 0xFFFFFFF00957C000 + 0x100;// 0xFFFFFFF009578000 + 0x100;
