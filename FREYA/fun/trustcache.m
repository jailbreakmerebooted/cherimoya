...
int loadTrustCacheBinaries(void) {
    printf("binaries.tc ret: 0x%llx", staticTrustCacheUploadFileAtPath([NSString stringWithFormat:@"%@%@", NSBundle.mainBundle.bundlePath, @"/iosbinpack/binaries.tc"], NULL));
    
    return 0;
}

int loadTrustCacheBinpack(void) {
    printf("tar.tc ret: 0x%llx", staticTrustCacheUploadFileAtPath([NSString stringWithFormat:@"%@%@", NSBundle.mainBundle.bundlePath, @"/iosbinpack/tar.tc"], NULL));
