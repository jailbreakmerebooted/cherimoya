...
mach_port_t get_send_once(mach_port_t recv) {
  ...
  if (err != KERN_SUCCESS) {
      printf("port right extraction failed: %s", mach_error_string(err));
    return MACH_PORT_NULL;
  }
    printf("made so: 0x%x from recv: 0x%x", so, recv);
  return so;
}
...
void xpc_crasher(char* service_name) {
  ...
  if(err != KERN_SUCCESS){
      printf("unable to look up %s", service_name);
    return;
  }
  ...
  if (err != KERN_SUCCESS) {
      printf("port allocation failed: %s", mach_error_string(err));
    return;
  }
  ...
  if (err != KERN_SUCCESS) {
      printf("port right insertion failed: %s", mach_error_string(err));
    return;
  }
